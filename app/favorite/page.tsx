    ...
    return(
        <section className="container mx-auto px-5 lg:px-10 mt-10">
            ...
            {data.length === 0 ? (
                ...
                description="Please add favourites to see them right here.."
                />
            ):(
                <div className="grid lg:grid-cols-4 sm:grid-cols-2 md:grid-cols-3 grid-cols-1 gap-8 mt-8">
                    {data.map((item)=>(
                        <ListingCard key={item.Home?.id}
                        ...
                        isInFavouriteList={item.Home?.Favourite.length as number >0 ? true : false}
                         />
                    ))}
                </div>
            )}
